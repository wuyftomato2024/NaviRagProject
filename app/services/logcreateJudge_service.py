...
# *****
# 功能 判断此次问题是否需要生成日志
# 说明 把问题传给ai来判断这次的问题是否需要生成日志 ，结果返回一个布尔值
# *****
...
def devLog_content_create(sql_db ,session_id ,user_id ,question ,model_flag ,openai_api_key):
    ...
    log_datas = json.loads(response.content)
    title = log_datas["title"]
    content = log_datas["content"]

    contextCreate(sql_db = sql_db ,session_id = session_id ,user_id = user_id ,content = content ,title = title )

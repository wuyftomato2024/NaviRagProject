...
# 说明
# *****
def devlogJudge_prompt():
    prompt = ChatPromptTemplate.from_messages([
            (
                "system", 
                """
                    请判断用户当前的问题，是否是在明确要求“生成/保存/写入一份开发日志”。

                    规则：
                    - 只有当用户明确表示想把当前对话、当前进展、当前内容整理成日志并保存/记录时，才返回 True
                    - 如果用户只是提到“日志”这个词，但并不是要求生成日志，返回 False
                    - 如果用户是在询问日志功能怎么实现、日志表怎么设计、日志报错原因，返回 False
                    - 如果用户只是普通提问、追问、学习某个知识点，返回 False
                    - 不明确时，返回 False

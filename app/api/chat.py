...
import os
from app.schemas.chat_context import ChatContext ,normalChatContext

# *****
# 功能 Chat逻辑
# 说明 先用词库拦截问题，直接判断需要什么模式，如果问题的文字刚好绕过再调用judge去判断需要用哪个模式回答
# *****
async def Chat(model_flag ,openai_api_key ,upload_file ,question ,top_k ,sql_db ,session_id ,user_id):
    ...
    normalContext = normalChatContext(
        question=question,
        openai_api_key=openai_api_key,
        sql_db=sql_db,
        session_id=session_id,
    ...
    if upload_file :
        ...
        response = await ragChat(ragContext)
        ...
        for history_kw in history_kws :
            if history_kw in question :
                response = normalChat(normalContext)
